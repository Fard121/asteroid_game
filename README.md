...
| Runtime component swap | Install / uninstall Player, Enemy and Weapon live with `1` / `2` / `3` |
...
```mermaid
flowchart TD
    Frame["AnimationTimer.handle"] --> Mute["Handle Mute key +<br/>component toggles (1/2/3)"]
...
sequenceDiagram
    ...
    else Enemy ship flies into an asteroid
        Collision->>World: removeEntity(enemy) — destroyed outright
        Collision->>Splitter: createSplitAsteroid(asteroid, world)
        Note over Score: no points — not a player kill
    end
```
